// ============================================================================
// MingleUp Bottom Navigation Bar (SUGO Style - Wallet ID reused for Moments)
// ============================================================================
...
export const BottomNav: React.FC = () => {
  ...
  const navItems: { id: NavTab; label: string; icon: React.ReactNode; badge?: number }[] = [
    ...
    {
      id: 'wallet', // نستخدم معرف wallet برمجياً لتجنب أي أخطاء في الـ TypeScript Types
      label: 'اللحظات',
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-30 pb-safe bg-white/85 dark:bg-slate-900/85 backdrop-blur-xl border-t border-slate-200/60 dark:border-slate-800/80 shadow-2xl transition-colors">
        ...
        {navItems.map(item => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[10px] sm:text-[11px] mt-0.5 tracking-tight font-medium">{item.label}</span>

              {/* Active Pill Indicator */}
              {isActive && (
                <span className="absolute -bottom-1 w-5 h-1 bg-gradient-to-r from-brand-600 to-rose-500 rounded-full shadow-sm shadow-brand-500/50" />
              )}
            </button>
          );
        })}
